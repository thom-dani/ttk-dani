...
namespace ttk {
  ...
  public:
    ...
    */
    template <typename dataType, typename triangulationType>
    void getIndices(triangulationType * triangulation,
    ...
    */
    ...
    inline void setOptimizationWithoutMatching(bool optimizationWithoutMatching) {
  ...
  protected:
    ...
    // Stopping criterion: when the loss becomes less than a percentage coefStopCondition_ (e.g. coefStopCondition_ = 0.01 => 1%) of the original loss (between input diagram and simplified diagram)
...
class PersistenceGradientDescent : public torch::nn::Module, public ttk::BackendTopologicalOptimization{
    public:
...
#pragma omp parallel for num_threads(threadNumber_)
  ...
  this->printMsg(
    "Get Indices | The number of vertices that need to be updated is : " + std::to_string(count),  debug::Priority::DETAIL);
  ...
  if(optimizationWithoutMatching_){
    for(SimplexId i = 0; i < (SimplexId)diagramOutput.size(); i ++){
      ...
      if((thresholdMethod_ == 0) && (pair.persistence() < thresholdPersistence_)){
      ...
      else if((thresholdMethod_ == 1) && ((pair.dim < lowerThreshold_) || (pair.dim > upperThreshold_))){
        birthPairToDeleteCurrentDiagram.push_back(static_cast<int64_t>(pair.birth.id));
        birthPairToDeleteTargetDiagram.push_back((pair.birth.sfValue + pair.death.sfValue) / 2);
        deathPairToDeleteCurrentDiagram.push_back(static_cast<int64_t>(pair.death.id));
        deathPairToDeleteTargetDiagram.push_back((pair.birth.sfValue + pair.death.sfValue) / 2);
    ...
    if(thresholdConstraintDiagram.size() == 0){
      ...
      for(const auto& entry : matchingsBlockPairs){
        ...
        if(entry.size() == 1){
          ...
          if(!constraintAveraging_){
            // If the point thresholdCurrentDiagram[entry[0]].birth.id is in a signal pair 
            // AND If the point thresholdCurrentDiagram[entry[0]].death.id is not in a signal pair 
            // Then we only modify the thresholdCurrentDiagram[entry[0]].death.id
            ...
            // If the point thresholdCurrentDiagram[entry[0]].death.id is in a signal pair
            // AND If the point thresholdCurrentDiagram[entry[0]].birth.id is not in a signal pair
            // Then we only modify the thresholdCurrentDiagram[entry[0]].birth.id
            ...
            // If the point thresholdCurrentDiagram[entry[0]].birth.id is in a signal pair
            // AND If the point thresholdCurrentDiagram[entry[0]].death.id is in a signal pair
            // Then we do not modify either point
...
/*
  ...
  and minimum for each axis and the number of coordinates per axis.
*/
std::vector<std::vector<double>> ttk::BackendTopologicalOptimization::getCoordinatesInformations(
...
int ttk::BackendTopologicalOptimization::execute(
  ...
  if(methodOptimization_ == 0){
    ...
    std::vector<std::vector<SimplexId>> pair2Delete(vertexNumber_, std::vector<SimplexId>());
    ...
    for(int it = 0; it < epochNumber_; it++){
      ...
      this->printMsg(
      "ExecuteOneBlock | DirectGradientDescent - iteration n° " + std::to_string(it),  debug::Priority::PERFORMANCE);
      // pairs to change
      std::vector<int64_t> birthPairToChangeCurrentDiagram{};
      std::vector<double> birthPairToChangeTargetDiagram{};
      std::vector<int64_t> deathPairToChangeCurrentDiagram{};
      ...
      getIndices(triangulation, inputOffsetsCopie, dataVector.data(), currentConstraintDiagram, it,
          listAllIndicesToChangeSmoothing, pair2MatchedPair, pair2Delete, pairChangeMatchingPair, birthPairToDeleteCurrentDiagram, 
          birthPairToDeleteTargetDiagram, deathPairToDeleteCurrentDiagram, deathPairToDeleteTargetDiagram, 
          birthPairToChangeCurrentDiagram, birthPairToChangeTargetDiagram, deathPairToChangeCurrentDiagram,
          deathPairToChangeTargetDiagram, currentVertex2PairsCurrentDiagram, vertexInHowManyPairs);
      ...
      std::vector<int64_t> &indexBirthPairToDelete = birthPairToDeleteCurrentDiagram;
      std::vector<double> &targetValueBirthPairToDelete  = birthPairToDeleteTargetDiagram; 
      std::vector<int64_t> &indexDeathPairToDelete = deathPairToDeleteCurrentDiagram; 
      std::vector<double> &targetValueDeathPairToDelete = deathPairToDeleteTargetDiagram; 

      this->printMsg(
      "ExecuteOneBlock | DirectGradientDescent - Number of pairs to delete : " + std::to_string(indexBirthPairToDelete.size()) ,  debug::Priority::DETAIL);

      std::vector<int> vertexInCellMultiple(vertexNumber_, -1); 
      std::vector<std::vector<double>> vertexToTargetValue(vertexNumber_, std::vector<double>()); 
          ...
          if(!(finePairManagement_ == 2) && !(finePairManagement_ == 1)){
            if(constraintAveraging_){
              if(vertexInHowManyPairs[indexMax] == 1){
                smoothedScalars[indexMax] = smoothedScalars[indexMax] - alpha_ * 2 * (smoothedScalars[indexMax]-targetValueBirthPairToDelete[i]);
                listAllIndicesToChangeSmoothing[indexMax] = 1;
              }
              else{
                vertexInCellMultiple[indexMax] = 1; 
                vertexToTargetValue[indexMax].push_back(targetValueBirthPairToDelete[i]); 
              }

              if(vertexInHowManyPairs[indexSelle] == 1){
                smoothedScalars[indexSelle] = smoothedScalars[indexSelle] - alpha_ * 2 * (smoothedScalars[indexSelle]-targetValueDeathPairToDelete[i]);
                listAllIndicesToChangeSmoothing[indexSelle] = 1;
              }
              else{
                vertexInCellMultiple[indexSelle] = 1; 
                vertexToTargetValue[indexSelle].push_back(targetValueDeathPairToDelete[i]); 
              }
            }
            else{
              smoothedScalars[indexMax] = smoothedScalars[indexMax] - alpha_ * 2 * (smoothedScalars[indexMax]-targetValueBirthPairToDelete[i]);
      ...
      this->printMsg(
      "ExecuteOneBlock | DirectGradientDescent - Loss Delete Pairs : " + std::to_string(lossDeletePairs) , debug::Priority::PERFORMANCE);
      ...
      std::vector<int64_t> &indexBirthPairToChange = birthPairToChangeCurrentDiagram;
      std::vector<double> &targetValueBirthPairToChange = birthPairToChangeTargetDiagram;
      std::vector<int64_t> &indexDeathPairToChange = deathPairToChangeCurrentDiagram;
      std::vector<double> &targetValueDeathPairToChange = deathPairToChangeTargetDiagram;
      ...
      for(size_t i = 0; i <  indexBirthPairToChange.size(); i++) {
        ...
        if(constraintAveraging_){
          if(vertexInHowManyPairs[indexMax] == 1){
            smoothedScalars[indexMax] = smoothedScalars[indexMax] - alpha_ * 2 * (smoothedScalars[indexMax]-targetValueBirthPairToChange[i]) ;
            listAllIndicesToChangeSmoothing[indexMax] = 1;
          }
          else{
            vertexInCellMultiple[indexMax] = 1; 
            vertexToTargetValue[indexMax].push_back(targetValueBirthPairToChange[i]); 
          }

          if(vertexInHowManyPairs[indexSelle] == 1){
            smoothedScalars[indexSelle] = smoothedScalars[indexSelle] - alpha_ * 2 * (smoothedScalars[indexSelle]-targetValueDeathPairToChange[i]);
            listAllIndicesToChangeSmoothing[indexSelle] = 1;
          }
          else{
            vertexInCellMultiple[indexSelle] = 1; 
            vertexToTargetValue[indexSelle].push_back(targetValueDeathPairToChange[i]); 
          }
        }
        else{
          smoothedScalars[indexMax] = smoothedScalars[indexMax] - alpha_ * 2 * (smoothedScalars[indexMax]-targetValueBirthPairToChange[i]) ;
      ...
      this->printMsg(
      "ExecuteOneBlock | DirectGradientDescent - Loss Change Pairs : " + std::to_string(lossChangePairs), debug::Priority::PERFORMANCE);
      ...
      if(constraintAveraging_){
        for(SimplexId i = 0; i < (SimplexId)vertexInCellMultiple.size(); i++){
          ...
          if(vertexInCellMultiple[i] == 1){
            ...
            averageTargetValue = averageTargetValue / (int)vertexToTargetValue[i].size(); 

            smoothedScalars[i] = smoothedScalars[i] - alpha_ * 2 * (smoothedScalars[i]-averageTargetValue);
      ...
      if(it == 0){
        stoppingCondition = coefStopCondition_ * (lossDeletePairs + lossChangePairs);
      }
  ...
  //=======================================
    ...
    for(int i = 0; i < epochNumber_; i++){
      ...
      getIndices(triangulation, inputOffsetsCopie, inputScalarsX.data(), currentConstraintDiagram, i,
              listAllIndicesToChange, pair2MatchedPair, pair2Delete, pairChangeMatchingPair, birthPairToDeleteCurrentDiagram, 
              birthPairToDeleteTargetDiagram, deathPairToDeleteCurrentDiagram, deathPairToDeleteTargetDiagram, 
              birthPairToChangeCurrentDiagram, birthPairToChangeTargetDiagram, deathPairToChangeCurrentDiagram, 
              deathPairToChangeTargetDiagram, currentVertex2PairsCurrentDiagram, vertexInHowManyPairs);
      ...
      torch::Tensor valueOfXDeleteBirth = torch::index_select(model.X, 0, torch::tensor(birthPairToDeleteCurrentDiagram));
      auto valueDeleteBirth = torch::from_blob(birthPairToDeleteTargetDiagram.data(), {static_cast<SimplexId>(birthPairToDeleteTargetDiagram.size())}, torch::kDouble);
      torch::Tensor valueOfXDeleteDeath = torch::index_select(model.X, 0, torch::tensor(deathPairToDeleteCurrentDiagram));
      auto valueDeleteDeath = torch::from_blob(deathPairToDeleteTargetDiagram.data(), {static_cast<SimplexId>(deathPairToDeleteTargetDiagram.size())}, torch::kDouble);
      ...
      torch::Tensor valueOfXChangeBirth = torch::index_select(model.X, 0, torch::tensor(birthPairToChangeCurrentDiagram));
      auto valueChangeBirth = torch::from_blob(birthPairToChangeTargetDiagram.data(), {static_cast<SimplexId>(birthPairToChangeTargetDiagram.size())}, torch::kDouble);
      torch::Tensor valueOfXChangeDeath = torch::index_select(model.X, 0, torch::tensor(deathPairToChangeCurrentDiagram));
      auto valueChangeDeath = torch::from_blob(deathPairToChangeTargetDiagram.data(), {static_cast<SimplexId>(deathPairToChangeTargetDiagram.size())}, torch::kDouble);
  ...
  this->printMsg(
          "Total execution time =  " + std::to_string(time) , debug::Priority::PERFORMANCE);

  // Number Pairs Constraint Diagram
  SimplexId numberPairsConstraintDiagram = (SimplexId)constraintDiagram.size();
  this->printMsg(
          "Number Pairs Constraint Diagram =  " + std::to_string(numberPairsConstraintDiagram) , debug::Priority::PERFORMANCE);


  this->printMsg(
          "Stop condition : " + std::to_string(stoppingCondition),  debug::Priority::PERFORMANCE);
